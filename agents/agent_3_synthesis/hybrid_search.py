...
class HybridSearchEngine:
    ...
    def index_chunks(self, chunks: List[DocumentChunk]):
        ...
        for chunk in chunks:
            # Create searchable text including context
            text = self._create_searchable_text(chunk)
            self._corpus.append(text)
            
            # Simple tokenization (can be improved with proper French tokenizer)
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        # Remove very short tokens and common French stopwords
        stopwords = {
    ...
    def hybrid_search(
        ...
        for rank, (chunk_id, combined_score) in enumerate(combined):
            ...
            # If not in our chunks, get from vector results
            if not chunk:

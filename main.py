...
# Configure logging - minimal output for better UX
logger.remove()
logger.add(
    sys.stderr,
...
def query_documents(workflow, question: str):
    ...
    # Affichage naturel/conversationnel
    print(f"\n🤖 Réponse:\n")
    print(f"{result.answer}\n")
    
    # Sources (optionnel, compact)
    if result.citations:
...
def interactive_mode(workflow=None, llm_service_type: str = "ollama"):
    ...
    if workflow is None:
        ...
        # Try to load existing indexed data from ChromaDB
    ...
    while True:
        try:
            ...
            if not workflow._indexed:
                print("⚠️  Aucun document indexé. Utilisez /index <chemin> d'abord.")
                continue
...
Exemples:
  ...
  python main.py --interactive
    ...
    parser.add_argument(
        ...
        type=str,
        default='ollama',
        choices=['otoroshi', 'openai', 'ollama', 'mock'],
    ...
    try:
        if args.interactive:
            ...
            print("⚠️  Veuillez d'abord indexer des documents avec --index")
            sys.exit(1)
